...
import employmentpo.ExtendedPositionInfoType;
...
public class ApplicantProfileGenerator {
            ...
            for (int i = 0; i < employeeRecords.size(); i++) {
                if (employeeRecords.get(i).getPersonalNumber().equals(profileModel.getPersonNumber())) {
                    ...
                    for (int j = 0; j < empRecords.size(); j++) {
                        String companyName = empRecords.get(j).getCompanyInfo().getName();
                        List<ExtendedPositionInfoType> positionList = empRecords.get(j).getPositions().getPosition();
    ...
    public void processEmploymentRecord(Records.Record record, ApplicantProfileXmlModel profileModel) {
        List<Records.Record.EmploymentRecord> empRecords = record.getEmploymentRecord();
        for (int i = 0; i < empRecords.size(); i++) {
            ...
            for (int j = 0; j < positionList.size(); j++) {
                String position = positionList.get(j).getPositionTitle();
                String startDate = positionList.get(j).getEntranceDate().toString();
                String endDate = positionList.get(j).getExitDate().toString();
                int hoursWeek = (int) positionList.get(j).getHoursPerWeek().intValue();
                String location = positionList.get(j).getLocation();
                String reasonExit = positionList.get(j).getExitReason();
                profileModel.addPositionDetails(companyName, position, startDate, endDate, hoursWeek, location, reasonExit);
            }
        }
    }
