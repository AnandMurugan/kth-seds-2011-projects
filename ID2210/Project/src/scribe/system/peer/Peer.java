...
import scribe.system.peer.scribe.PublishEvent;
import scribe.system.peer.scribe.Scribe;
import scribe.system.peer.scribe.ScribeInit;
import scribe.system.peer.scribe.ScribePort;
...
public final class Peer extends ComponentDefinition {
	...
	private Component cyclon, tman, scribe, bootstrap;
...
//-------------------------------------------------------------------	
	public Peer() {
		...
		connect(tman.getPositive(TManPartnersPort.class), scribe.getNegative(TManPartnersPort.class));
		
		subscribe(handleInit, control);
		subscribe(handleJoin, peerPort);
		subscribe(handlePublish, peerPort);
...
//-------------------------------------------------------------------	
	Handler<PeerInit> handleInit = new Handler<PeerInit>() {
		public void handle(PeerInit init) {
			...
			tmanConfiguration = init.getApplicationConfiguration();
...
//-------------------------------------------------------------------	
	Handler<JoinPeer> handleJoin = new Handler<JoinPeer>() {
		public void handle(JoinPeer event) {
			...
			Snapshot.addPeer(peerSelf, topics);
		}
	};
...
//-------------------------------------------------------------------	
	Handler<PublishPeer> handlePublish = new Handler<PublishPeer>() {
		public void handle(PublishPeer event) {
			trigger(new PublishEvent(event.getTopicId()), scribe.getPositive(ScribePort.class));
		}
	};
//-------------------------------------------------------------------	
...
//-------------------------------------------------------------------	
	Handler<JoinCompleted> handleJoinCompleted = new Handler<JoinCompleted>() {
		public void handle(JoinCompleted event) {
			...
			trigger(new ScribeInit(peerSelf, topics, tmanConfiguration.getPeriod()), scribe.getControl());
